\documentclass[a4paper, 11pt, oneside, showtrims]{book}
...
% todo:
% pisu vsude starym nebo novym pravopisem? --novym, ale ne vsude!
% vymyslet, jak rozjet Visual Browser nebo jeho nefunkcnost popsat diplomaticky (a smazat pak teda příkladem budiž Visual Browser zde rozebíraný)

% nazev rozhrani: not so fickle/frenzy/??? wordnet viewer
	% RAW - Responsive Wordnet
	% Yet Another Perfect Wordnet Visualization in Graphs
	% RWV - responsive wordnet viewer
	% RAW - Responsively Awesome/ adequate / academic / advanced wordnet viewer
% doimplementovat, aby rozhrani kontrolovalo, jestli parametry v~url zna
% udelat v~kodu konfigurovatelnou abulafiu (dotazy na server)
...
\dominitoc % init of mini table of contents for chapter TOC (see chapt. about comparing WN interfaces)

\begin{document}

\todo[inline]{TODO tady v~textu:}
	\todo[inline,backgroundcolor=red!25]{prosim kontrolu, ze pisu novym pravopisem ("responzivní místo responsivní") -- když narazíš na něco starým, prosím, flaguj/opravuj to}
	\todo[inline,backgroundcolor=red!25]{reference na kapitoly by vsude mely byt v kurzive (jejich jmena)}
	\todo[inline,backgroundcolor=red!25]{název rozhraní ještě není finální, nějaké nápady, jako ho vylepšit? (původní nápad Not So Fickle Wordnet viewer, další Not Safe For Work wordnet viewer), momentalne RAW viewer -> Responsively Awesome Wordnet viewer, to by chtelo neco misto toho "awesome" ale od A}

\par
\todo[inline]{TODO v~samotném rozhraní:}
	\todo[inline,backgroundcolor=red!25]{doimplementovat, aby rozhrani kontrolovalo, jestli parametry v~url zna}
	\todo[inline,backgroundcolor=red!25]{udelat v~kodu konfigurovatelnou abulafiu (dotazy na server)}
	...
	{\setstretch{1.12}
		...
		\adjustmtc % to make minitoc print right chapter 
		\adjustmtc % more chapters wih asterisk (no numbering) means we have to add this multiple times
	}
	...
	\label{part:eins}
		...
		\label{cha:princeton_wn}
			...
			\label{cha:psycho}
				...
				WordNet se pro svou podobnost s~lidskou pamětí v~uspořádání informací zdá být vhodný nejen k~úkolům počítačového zpracování jazyka, ale i k~činnostem v~jiných oblastech. V~souladu s~příklady, jež byly předloženy výše, se zdá, že zkoumání významů a propojenosti konceptů v~jazyce, s~nímž není uživatel zcela obeznámen, by mohlo ve WordNetu být snazší než v~klasickém slovníku. V~podobném duchu využili při svých kursech WordNet například \textcite{lemnitzer2003using}.
			...
			\label{cha:lexvztah}
				...
				\label{cha:synon}
					...
					Synonymie je základním definičním vztahem pro synsety ve WordNetu. Na praktických aplikacích je tento jev nejlépe pozorovatelný, jelikož při vyhledání konkrétní formy je uživateli obvykle nabídnut výběr z~jednotlivých významů dané formy. Aby byly od sebe významy oné formy odlišitelné, nabídka obvykle sestává ze seznamu skupin slovních forem náležejících do nalezených synsetů\td{tu footnote potom smazat, ale prislo mi to vtipne :)}\footnote{\url{https://www.englishforums.com/English/AdjectiveSatellite/nwzhv/post.htm#1126701}} Kupříkladu při vyhledání slova \ex{kolo} v~českém wordnetu tak je uživatel konfrontován s~několika skupinami, které obsahují zhruba následující:
		...
		\label{cha:dalsi_wordnety}
			...
			\label{cha:balkanet}
				...
				BalkaNet je projekt zaštítěný Evropskou komisí, který začal být vyvíjen v~roce 2001. Cílem BalkaNetu je vytvořit ucelený lingvistický zdroj pro méně studované balkánské jazyky. Jde o soubor jednotlivých wordnetů pro jazyky bulharština, rumunština, řečtina, srbština a turečtina. Do projektu spadá také rozšíření českého wordnetu vytvořeného v~rámci projektu EuroWordNet, a to zejména o slovesa. Při vzniku BalkaNetu byli pozváni účastníci vzniku EuroWordNetu z~Čech a Francie, aby mohli být nápomocni s~vytvořením struktury dat takové, jaká by byla kompatibilní s~existujícími daty EuroWordNetu. \parencite{tufis2004balkanet}

				Nejvyšším cílem BalkaNetu je vytvořit online reprezentaci sémantických relací v~jazycích balkánských států a poskytnout tak zdroj pokrývající běžnou slovní zásobu těchto jazyků. BalkaNet je tvořen wordnety, jež mají stejnou strukturu jako wordnety EuroWordNetu, aby bylo možné jej považovat díky kompatibilitě s~ním za rozšíření existující práce. 

				Stejně jako EuroWordNet používá BalkaNet za účelem provázání jednotlivých svých wordnetů tzv. mezijazykový index. Na rozdíl od EuroWordNetu jej však používá mírně jinak v~tom ohledu, že jsou koncepty označkovány doménovými štítky\footnote{\textit{domain labels}}. Koncepty jsou tedy mezi národním wordnetem a mezijazykovým konceptem provázány tak, aby zůstala zachována významová doména a předešlo se tak svazování konceptů z~různých domén přes tento index. Pokud by tedy BalkaNet byl použit jako překladový nástroj mezi dvěma obsaženými jazyky, výsledné překladové koncepty nemusí nutně být přímé překlady zdrojových, ale budou alespoň náležet do stejné konceptuální domény. \parencite{christodoulakis2002balkanet} Celkově je mezijazykový index strukturalizován na rozdíl od toho v~EuroWordNetu jako princetonský WordNet právě proto, aby bylo možné svazek wordnetů v~BalkaNetu lépe užívat k~lingvistickým úkolům. \parencite{tufis2004balkanet}
	...
	\label{part:zwei}
		...
		\label{cha:porovnani}
			...
			\minitoc % mini TOC of this chapter
			...
			% https://tex.stackexchange.com/questions/312979/removing-renaming-the-title-of-minitoc
			...
			\section{Vizualizace s~webovým rozhraním}
				...
				\label{wnvis:intviswn}
					...
					An interactive visualization of the Princeton WordNet database, dostupný z~\url{http://mateogianolio.com/wordnet-visualization/}, je jeden z~projektů programátora jménem Mateo Gianolio. Jde o jednoduché rozhraní napojené na princetonský WordNet, které po zadání hledaného výrazu zobrazí synsety obsahující daný výraz. Jednotlivé synsety jsou barevně odlišeny podle syntaktických kategorií (slovních druhů), k~nimž náležejí, a uspořádány kruhově kolem hledaného výrazu. Z~každého synsetu je vždy zobrazena jen první slovní forma. Pokud uživatel najede kursorem myši na některý ze synsetů, zobrazí se další případné slovní formy náležející do daného synsetu a jeho glosa (definice), je-li dostupná. Vzhled rozhraní je ilustrován na obrázku \ref{fig:intviswn} na straně \pageref{fig:intviswn}
			...
			\section{Vizulizace v~aplikačním prostředí Java}
				...
				Java je objektově orientovaný programovací jazyk, který je zaměřený na to, aby měl co nejméně závislostí na konkrétním operačním systému a technickém vybavení počítače, jak jen to je možné. To je výhodné zejména kvůli konceptu zvanému WORA\footnote{Write Once, Run Anywhere, tedy napsat jednou, spustit kdekoliv}, protože to znamená, že skompilované programy napsané v~Javě lze spustit na jakémkoliv stroji, který podporuje Javu. Takto skompilované programy se spouští v~aplikačním prostředí zvaném Java Virtual Machine\footnote{angl. virtuální stroj Javy} (JVM), jež vytváří uniformní prostředí pro běh aplikací. Výhodou Javy tedy je velká přenositelnost programů v~tomto jazyce napsaných, jelikož jednou skompilovaný kód lze spustit na různých operačních systémech a zařízeních. Nevýhodou je, že si uživatel, který chce program v~Javě napsaný spustit, musí nainstalovat JVM. 
			...
			\subsection{GoldenDict}
				...
				Podobně jako Artha funguje GoldenDict, dostupný z~\url{http://goldendict.org}, který, jsa univerzálnějším rozhraním pro více lexikografických zdrojů, byl testován jen ve své verzi pro WordNet. GoldenDict zobrazuje čistě textovou reprezentaci umožňující kliknutím zobrazit detaily jednotlivých synsetů. Slovníková data si obstarává aplikace sama, takže uživatel nemusí podstupovat instalaci dat princetonského WordNetu (která může být netriviální, protože například pro Windows neposkytují oficiální webové stránky instalátor pro data WordNetu ve verzi 3). Zároveň však rozhraní umožňuje přidávat další slovníky a v~základu umožňuje zobrazovat výsledky hledání z~Wikipedie. 
		...
		\chaptermark{Shrnutí přehledu}
			...
			% drtiva vetsina rozhrani neni responzivni, coz je problem, kdyz vetsina lidi pouziva web z~mobilu: http://bgr.com/2016/11/02/internet-usage-desktop-vs-mobile/
	...
	\label{part:drei}
		...
		\label{cha:techno}
			...
			\section{HTML}
				...
				Značkovací jazyk HTML je základním stavebním prvkem většiny dokumentů pro webové prohlížeče. Jako značkovací jazyk umožňuje HTML definici obsahu dokumentu, jeho strukturalizaci do částí, definici významů jednotlivých částí (například označení nadpisu) a do jisté míry jej lze užívat, byť k~tomu není určeno \parencite{cssWeb}, i pro definici vzhledu dokumentu (tučný řez písma, etc.). Poskytuje také prostředky pro tzv. hypertextové odkazy, jimiž lze provázat různé dokumenty či části jednoho dokumentu. Nezřídka se tyto odkazy v~dnešní době používají jako ovládací prvky k~řízení změn obsahu dynamicky generovaného dokumentu, kterým je například zde popisované rozhraní \simplywn. 

				V rozhraní \simplywn HTML slouží k~definici struktury zobrazené webové stránky, jejímu rozdělení na jednotlivé sekce (levý sloupec, pravá plocha, záhlaví, etc.) a ke strukturalizaci posléze zobrazovaných informací. Příkladem může být výpis sémantických relací, v~nichž je zobrazovaný synset přítomen. Jednotlivé relace jsou rozděleny do bloků (jež jsou později pomocí CSS nastaveny tak, aby se zobrazovaly jako sloupce) a jednotlivé synsety svázané s~tím zobrazovaným příslušným vztahem jsou vypsány v~seznamu. 
...
[
    { // první nalezený synset
        "id": "ENG20-02759263-n",
        "pos": "n",
        "def": "definice synsetu",
        "synset": [ // detaily synsetu
            {
                "name": "kotouč",
                "meaning": "2"
            },
        ...
        "paths": [ // cesty k~synsetu
            { // první cesta
                "name": "path-1",
                "breadcrumbs": [
                    { // nejvyšší synset v~cestě
                        "id": "ENG20-00020486-n",
                        "def": "",
                        "synset": [
                            {
                    ...
                    // ... další synsety v~cestě
                ],
            },
            // ... další alternativní cesty
        ],
        "children": [ // sémantické relace synsetu
            { // relace hyponymie
                "name": "hyponym",
                "children": [
                    {
