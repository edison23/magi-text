...
\todo[inline]{TODO tady v textu:}
	\todo[inline,backgroundcolor=red!25]{kontrola, ze pisu novym pravopisem ("responzivní místo responzivní") -- když narazíš na něco starým, prosím, flaguj to}
	\todo[inline,backgroundcolor=red!25]{pomoc, jak rozjet Visual Browser, nefunguje mi ani s RDF z adresy na jeho webu}
	\todo[inline,backgroundcolor=red!25]{název rozhraní ještě není finální, nějaké nápady, jako ho vylepšit? (Not So Fickle Wordnet viewer)}
	...
	\label{part:eins}
		...
		\label{cha:princeton_wn}
			...
			\label{cha:psycho}
				...
				Navzdory tomu, že je WordNet (a všeobecně wordnety) využíván převážně k~úkolům počítačového zpracování jazyka\td{nedari se mi k tomu najit citaci, jen to tusim, any help?}, zdá se být pro svou podobnost v uspořádání informací vhodný i v jiných oblastech. V souladu s~příklady, jež byly předloženy výše, se zdá, že zkoumání významů a propojenosti konceptů v jazyce, s~nímž není uživatel zcela obeznámen, by mohlo ve WordNetu být snazší než v klasickém slovníku. V podobném duchu využili při svých kursech WordNet například \textcite{lemnitzer2003using}.
				...
				Štítek \textit{non-factive} je používán u kauzální relace, která nemusí být nutně naplněna:
					...
					\begin{table}[h!]
						...
						\begin{tabular}{lllc|c}
							příčina	& vztah  & non-factive? & následek & nutně vyplývá? \\ \hline
							\ex{zabít}  & vyústí v & –  & \ex{zemřít} & ano                       \\
							\ex{hledat} & vyústí v & \texttt{non-factive} & \ex{najít}  & ne
						\end{tabular}
		...
		\label{cha:porovnani}
			...
			\section{Vizulizace v aplikačním prostředí Java}
				...
				\subsection{VisualBrowser}
					...
					\td{napsat diplomaticteji, nebo se pokusit aplikaci rozjet poradne}Verze aplikace VisualBrowser, dostupná z \url{https://nlp.fi.muni.cz/projekty/visualbrowser}, testovaná v rámci této práce neprokázala dostatečnou intuitivitu a funkčnost, aby její vlastnosti mohly být dále rozebrány. 
